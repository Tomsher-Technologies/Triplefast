        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="breadcrumb-main">
                    <h4 class="text-capitalize breadcrumb-title">Update Role</h4>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card card-default card-md mb-4">
                        ...
                        {!! Form::model($role, ['method' => 'PATCH','autocomplete' => 'off','route' => ['roles.update', $role->id]]) !!}
                                @csrf
                                <div class="form-row mx-n15">
                                    <div class="col-md-4 mb-20 px-15">
                                        ...
                                        <input type="text" class="form-control ih-medium ip-light radius-xs b-light" id="name"  name="name" placeholder="Enter name" value="{{ old('name', $role->name) }}">
                                        @error('name')
                                    ...
                                    <div class="col-md-4 mb-20 px-15">
                                        ...
                                        <select class="custom-select form-control select-arrow-none ih-medium  radius-xs b-light shadow-none color-light  fs-14" id="user_type" name="user_type">
                                            ...
                                            @foreach($user_types as $type)
                                                <option value="{{ $type->id }}" @if(old('user_type', $role->type) == $type->id) selected @endif>{{ $type->title }}</option>
                                            @endforeach
                                        </select>
                                        @error('user_type')
                                            <div class="alert alert-danger">{{ $message }}</div>
                                            ...
                                            @foreach($permission as $value)
                                                ...
                                                <div class="col-md-3">
                                                    <label class="fs-14 fw-400">
                                                        <input class="checkbox name" type="checkbox" value="{{$value->id}}" name="permission[]" id="permission" @if($selected != '') checked="checked" @endif>
                                                        {{ $value->title }}
                                                    </label>
                                                </div>
                                            @endforeach
